...
static __inline__ void g2x_Info(void)
{
  ...
  static int x,y;
    ...
    y=curheight-16;
    g2x_SetFontSize(12);
    y-=16; g2x_WriteStatic("Librairie G2X - version 5.0 - Dec.2012/Jan.2013",x,y,(float*)G2Xk);
    y-= 4; g2x_WriteStatic("_______________________________________________________________",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[ESC],[Ctrl+q] : quitte le programme proprement (liberation)   ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[SPACE] : stoppe/relance l'animation                           ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[?] : affiche cette fenetre d'aide                             ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[Ctrl+f] : plein ecran                                         ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[Ctrl+g] : affiche la grille de coordonnées                    ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[Ctrl+w] : inverse la couleur de fond                          ",x,y,(float*)G2Xk);
    y-=16; g2x_WriteStatic("[+],[-],[=] : facteurs de zoom ([=]:1.)                        ",x,y,(float*)G2Xk);
    y-= 4; g2x_WriteStatic("_______________________________________________________________",x,y,(float*)G2Xk);
    glColor3f(1.,1.,1.);
    ...
    while (scroll<_SCROLLH_+scrollhnum)
    {
      ...
      y-=12; g2x_WriteStatic(msg,x,y,(float*)G2Xk);
      scroll++;
    }
    scroll=_SCROLLV_;
...
extern void g2x_InitWindow(char* windname, int w, int h)
{
  ...
  snprintf(_WINDNAME_,127,"%s",windname);
...
{ 
  ...
  /* zone de crontôle horizontale : en bas - les scroll horiz. */
  if (_SCROLLH_)
  { 
    xdialflag=true;
  ...
  /* zone de contrôle verticale : à droite - les scroll vertic. et les switch, bouttons et popup */
...
extern void   g2x_tempo(double tempo) 
{
  char command[64]="";
