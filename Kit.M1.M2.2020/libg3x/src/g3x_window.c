...
/*=                        tracé des axes                            =*/
/*====================================================================*/
extern void g3x_Arrow(GLfloat r, GLfloat h, const float* col)
{
  static GLUquadricObj *qobj=NULL;
  glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
  glPushMatrix();
    if (!qobj) qobj=gluNewQuadric();
    glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT,(const float[4]){0.,0.,0.,0.});
...
extern void g3x_ArrowVect(G3Xpoint O, G3Xvector V, GLfloat r, GLfloat h, const float* col)
{
  static GLUquadricObj *qobj=NULL;
  if (!qobj) qobj=gluNewQuadric();
  glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
  ...
  glPushMatrix();
    ...
    glPushMatrix();
      ...
      glPushMatrix();
        ...
        glPushMatrix();
          glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT,(const float[4]){0.1, 0.4, 0.0, 1.0});
          glMaterialfv(GL_FRONT_AND_BACK,GL_DIFFUSE,col);
...
{
  glPushMatrix();
    g3x_Arrow(0.025,1.0,(const float[4]){0.0,0.0,0.6,1.0}); /*axe z (bleu) */
    glPushMatrix();
      glRotatef(-90.,1.,0.,0.);
      g3x_Arrow(0.025,1.0,(const float[4]){0.0,0.6,0.0,1.0}); /* axe y (vert) */
    glPopMatrix();  
    glPushMatrix();
      glRotatef(+90.,0.,1.,0.);
      g3x_Arrow(0.025,1.0,(const float[4]){0.6,0.0,0.0,1.0}); /* axe x (rouge) */
    glPopMatrix();  
  glPopMatrix();  
}
...
static __inline__ void g3x_Info(bool i)
{
  ...
  if (i)
  {
    ...
    glColor3f(1.,1.,1.);
    sprintf(tmp,"pos.cart. "); g3x_Print(60,y,tmp); sprintf(tmp,"[%+3.3f,%+3.3f,%+3.3f]",spot.pos[0],spot.pos[1],spot.pos[2]); g3x_Print(150,y,tmp);  y-=12;
...
static __inline__ void g3x_DrawDisplay(void)
{
  ...
  glEnable(GL_LIGHTING);
  glPolygonMode(GL_FRONT_AND_BACK,_FILL_?GL_FILL:GL_LINE);
  GLfloat _POS[4]={spot.pos[0],spot.pos[1],spot.pos[2],1. };
...
extern void g3x_GetMousePosition(G3Xpoint mousepos)  { memcpy(mousepos,_CLIC_POS_,sizeof(G3Xpoint)); }
...
{ 
  glutSetWindow(mainwin);
    ...
    { 
      ...
      case 'j' : g3x_PlugCapture(_WINDNAME_,0,0,curwidth,curheight); g3x_Snapshot("jpg",_WINDNAME_,curwidth,curheight); break;
      case 'J' : g3x_PlugCapture(_WINDNAME_,0,0,curwidth,curheight); g3x_Snapshot("png",_WINDNAME_,curwidth,curheight); break;
